...
namespace CP {
    ...
    public:
        ...
        int height(node *n) {
            ...
            if (n->left == nullptr) return 1 + height(n->right);
            if (n->right == nullptr) return 1 + height(n->left);
            return 1 + std::max(height(n->left), height(n->right));
        }

        int balance_val(node *n) {
            int a = height(n->left) - height(n->right);
            return a < 0 ? -a : a;
        }

        void traverse(node *n, std::pair<int, KeyT> &p) {
            if (n == nullptr) return;
            int b = balance_val(n);
            if (b > p.first)
                p = {b, n->data.first};
        ...
        KeyT getValueOfMostImbalanceNode() {
            std::pair<int, KeyT> ret = {0, begin()->first};
            traverse(mRoot, ret);
            return ret.second;
        }
        // END CODE
